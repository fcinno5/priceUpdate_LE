...
async function main() {
  try {
    ...
    const LE_LIST = await playerSearch([840], 0); // playerSearch(시즌넘버, 최소오버롤)
    let LE_RESULTS = await playerPriceValue(LE_LIST, [1, 2, 3, 4, 5, 6, 7, 8]); // playerPriceValue(데이터 , 강화등급)
    await saveToDB(LE_RESULTS);
